...
static int humidifierState = -1;  // -1 = off, 0-2 = estados
static unsigned long lastHumidifierChange = 0;
static const unsigned long HUMIDIFIER_INTERVAL = 10000;  // 10 segundos
static int cyclesCompleted = 0;
static bool inRest = false;
static unsigned long restStart = 0;

void ActuatorController::initHumidifierSeries() {
    // Energizar módulo maestro primero  
    turnHumidifierMasterOn();
    Serial.println("⚡ [HUMID] Energizando humidificador...");
    lastHumidifierChange = millis() + 2000;  // Warmup 2s
    humidifierState = 0;
    inRest = false;
}

void ActuatorController::runHumidifierRoutine(unsigned long durationMs) {
    // For compatibility, start the series once
    static bool seriesStarted = false;
    if (!seriesStarted) {
        initHumidifierSeries();
        seriesStarted = true;
    }
    
    // Run the state machine (non-blocking)
    humidifierTask();
}

void ActuatorController::humidifierTask() {
    unsigned long currentMillis = millis();
    
    // Handle rest period
    if (inRest) {
        if (currentMillis - restStart >= 10000) {
            Serial.println("🔄 [HUMID] Reiniciando serie después del descanso...");
            initHumidifierSeries();
        }
        return;
    }
    ...
    if (humidifierState >= 0 && currentMillis - lastHumidifierChange >= HUMIDIFIER_INTERVAL) {
        lastHumidifierChange = currentMillis;
        
        switch (humidifierState) {
            case 0: // Estado 1: Encender
                Serial.println("🌫 [HUMID] Estado 1: Pulso para encender (1s)");
                turnHumidifierRelayOn();
                delay(1000);
                turnHumidifierRelayOff();
                humidifierState = 1;
                break;
                
            case 1: // Estado 2: Mantener  
                Serial.println("⏳ [HUMID] Estado 2: Pulso para mantener encendido (1s)");
                turnHumidifierRelayOn();
                delay(1000);
                turnHumidifierRelayOff();
                humidifierState = 2;
            ...
            case 2: // Estado 3: Apagar
                Serial.println("🛑 [HUMID] Estado 3: Pulso para apagar (1s)");
                turnHumidifierRelayOn();
                delay(1000);
                turnHumidifierRelayOff();
                ...
                if (cyclesCompleted >= 2) {
                    // Serie completa - descanso
                    Serial.println("💤 [HUMID] Serie completada, apagando por 10s...");
                    ...
                    humidifierState = 0; // Reiniciar ciclo
                }
                break;
        }
    }
