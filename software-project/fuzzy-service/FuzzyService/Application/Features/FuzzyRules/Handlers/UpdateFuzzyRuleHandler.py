...
class UpdateFuzzyRuleHandler(CommandHandler[UpdateFuzzyRuleCommand]):
    ...
    async def __call__(self, request: UpdateFuzzyRuleCommand) -> None:
        ...
        routine_repository = di[IFuzzyRoutineRepository]
        ...
        # Actualizar consecuente si se proporciona
        if request.consequent is not None:
            # Verificar que la rutina existe
            routine = await routine_repository.get_by_id(request.consequent)
            if not routine:
                raise EntityNotFoundError(f"Rutina con ID {request.consequent} no encontrada")
            
            existing_rule.update_consequent(request.consequent)
