...
class RoutineStepDto(BaseModel):
    ...
    step_id: int = Field(
        ..., 
        ge=0,
        description="ID único del paso dentro de la rutina"
    )
    condition: str = Field(
        ..., 
        min_length=1,
        max_length=200,
        description="Condición que debe cumplirse para ejecutar este paso"
    )
    power_tag_id: str = Field(
        ..., 
        description="ID del tag de potencia"
    )
    duration_tag_id: str = Field(
        ..., 
        description="ID del tag de duración"
    )

    @field_validator('condition')
    @classmethod
    def validate_condition(cls, v: str) -> str:
        """Valida que la condición no esté vacía."""
        v = v.strip()
        if not v:
    ...
    def validate_tag_ids(cls, v: str) -> str:
        """Valida que los IDs de tags sean ObjectIds válidos."""
    ...
    def from_entity(cls, step: RoutineStep) -> RoutineStepDto:
        ...
        return cls(
            ...
            power_tag_id=str(step.power_tag_id),
            duration_tag_id=str(step.duration_tag_id)
        )
    ...
    def to_entity(self) -> RoutineStep:
        ...
        return RoutineStep(
            ...
            power_tag_id=self.power_tag_id,
            duration_tag_id=self.duration_tag_id
        )
...
class FuzzyRoutineDto(BaseModel):
    ...
    def validate_routine_steps_order(self) -> 'FuzzyRoutineDto':
        ...
        if self.steps:
            ...
            # Validar que los step_ids estén ordenados
...
class UpdateRoutineStepDto(BaseModel):
    """DTO para actualizar un paso específico en una rutina difusa."""
